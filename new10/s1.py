...
def announce_leader():
    ...
    # Sicherstellen, dass der neue Leader die verbundene Clients-Liste kennt
    if not connected_clients:
        print("No clients connected. Leader will wait for new connections.")
    else:
        # Broadcast an alle Clients, um sie über den neuen Leader zu informieren
        leader_message = json.dumps({
        ...
        for client_id, client_sock in connected_clients.items():
            try:
                client_sock.send(leader_message)
                print(f"Notified client {client_id} about the new leader")
            except Exception as e:
