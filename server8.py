...
UDP_PORT = 42000
BUFFER_SIZE = 1024
HEARTBEAT_INTERVAL = 10  # Send heartbeat every 5 seconds
HEARTBEAT_TIMEOUT = 40  # Timeout if no heartbeat received within 10 seconds
...
def broadcastServerPresence(tcpPort):
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udpSocket:
        ...
        while isActive:
            ...
            time.sleep(15)  # Broadcast every 15 seconds instead of 10
        ...
        for serverPort, serverAddr in list(connectedServers.items()):
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcpSocket:
                    print(f"Attempting to send heartbeat to {serverPort}")
                    ...
                    print(f"Heartbeat sent to {serverPort}")
            except Exception as e:
                print(f"Error sending heartbeat to {serverPort}: {e}")
                missedHeartbeats[serverPort] = missedHeartbeats.get(serverPort, 0) + 1
                if missedHeartbeats[serverPort] >= 3:
                    attemptReconnection(serverPort, serverAddr)


def attemptReconnection(serverPort, serverAddr):
    reconnectAttempts = 5  # Number of reconnection attempts
    waitTime = 2  # Start with a 2-second wait
    for attempt in range(reconnectAttempts):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as tcpSocket:
                tcpSocket.connect((serverAddr[0][0], serverPort))
                print(f"Reconnected to server {serverPort} on attempt {attempt + 1}.")
                missedHeartbeats[serverPort] = 0  # Reset missed heartbeats after successful reconnection
                return  # Exit function if reconnection is successful
        except Exception as e:
            print(f"Reconnection attempt {attempt + 1} to server {serverPort} failed: {e}")
        time.sleep(waitTime)  # Increase wait time after each failed attempt
        waitTime *= 2  # Exponential backoff



# Monitor heartbeats from servers to detect failures
# Monitor heartbeats from servers to detect failures and attempt reconnection
    ...
    while isActive:
        ...
        for serverPort in list(lastHeartbeat.keys()):
            if currentTime - lastHeartbeat[serverPort] > HEARTBEAT_TIMEOUT:
                print(f"Server {serverPort} has missed heartbeats or is unreachable.")
                
                # If the leader has failed, trigger an election
                if leader and leader[1] == serverPort:
                    ...
                    initiateLeaderElection()  # Trigger election if the leader fails
                else:
                    print(f"Attempting to reconnect to server {serverPort}.")
                    attemptReconnection(serverPort, connectedServers[serverPort])
