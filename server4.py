    ...
    while True:
        try:
            ...
            if messageType == "client_id":
                ...
                if action == "join":
                    print(f"Client {clientId} joining chatroom {chatroom}")
                elif action == "message":
                    print(f"Message from {clientId}: {data['content']}")
                elif action == "leave":
                    print(f"Client {clientId} leaving chatroom {chatroom}")
...
def main():
    global serverId, isActive, leader
