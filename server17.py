...
LEADER_CLIENT_PORT = 60004  # Dedicated port for leader-client communication
...
class Server:
    def __init__(self):
        ...
        self.leader_client_socket = None
    ...
    def become_leader(self):
        ...
        # Set up leader-client communication channel
        self.leader_client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.leader_client_socket.bind(('0.0.0.0', LEADER_CLIENT_PORT))
        self.leader_client_socket.listen(5)
            ...
            try:
                ...
                if data:
                    ...
                    if message == "HEARTBEAT":
                        ...
                        self.broadcast_to_clients(f"Client {client_address}: {message}")
                else:
                    break
            except socket.error:
                break
    ...
    def broadcast_leader_info(self):
        ...
        while self.running and self.is_leader:
            message = f"LEADER:{self.leader_id}:{self.leader_ip}:{LEADER_CLIENT_PORT}"
