...
# Function to generate unique ID using MAC address, timestamp, and random number
def generate_unique_id():
    ...
    # Combine MAC address, timestamp, and random number into a unique ID
    unique_id = f"{mac_address}-{timestamp}-{random_number}"
...
# Function to listen for leader announcement via UDP
def listenForLeader():
        ...
        while True:
            try:
                ...
                if messageType == "leader_announcement":
                    leaderIp = data['leader_ip']  # Get the leader IP (string)
                    leaderPort = int(data['leader_port'])  # Get the port
                    return leaderIp, leaderPort  # Return IP and port
...
# Function to reconnect to the leader
...
def main():
    ...
    try:
        ...
        sendMessageToServer(tcpSocket, "client_id", client_id=myId)

        threading.Thread(target=receiveMessages, args=(tcpSocket,)).start()

        # Client menu for sending messages, joining chatrooms, etc.
        while True:
            print("\nChoose an option:")
